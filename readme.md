...
also a search option for imdrf codes for the report of adverse events by the prrc is included. fortunately the [imdrf-website](https://www.imdrf.org/documents/terminologies-categorized-adverse-event-reporting-aer-terms-terminology-and-codes) provides json-files!
as of 3/23 these are requested on the fly so you never have outdated versions.

<details>
<summary>using local files and updateing manually...</summary>
the json-files have to be altered by name (e.g. `regulatorytools.data.imdrf.annex.a.js`) and have the content assigned to the regulatorytools.data-object by inserting
```Javascript
