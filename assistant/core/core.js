	...
	static: { // these methods do not depend on asynchronous calls
		drm: {
			...
			decryptToken: function (hashTable, base36Timestamp, token) {
				...
				for (let i = core.var.drm.pwLength.min; i < core.var.drm.pwLength.max; i++) {
		...
		escapeHTML: (text, br2nl) => { //primary use for escaping special chars for mailto
		...
		insert: { //handle repetitive design patterns
			...
			select: (options, name, id, selected, additionalProperty) => {
				// output has to be object with optionId:[value,label] pairs
				let output = '<select name="' + name + '" id="' + id + '" ' + (additionalProperty ? additionalProperty : '') + '>'
				if (options != null)
		...
		maxMailSize: () => {
			...
			if (confirm(core.fn.static.lang('settingMailSizeDeterminationHint'))) {
				for (let i = core.var.directMailSize; i > 256; i--) {
					good = 1;
					num = '';
					for (let j = 0; j < i; j++) num += 'a';
					try {
		...
		selectText: (element) => { //selection of output-content on click if not disabled
			let doc,
				range,
				selection,
				text,
		...
		sortBySecondColumn: (a, b) => { //for two-dimensional arrays
			if (a[1] === b[1]) {
				return 0;
			} else {
				return (a[1] < b[1]) ? 1 : -1;
	...
	async: { // these methods depend on asynchronous calls at some time
		...
		loadScript: async (url, callback) => { //load given script-files into scope, e.g. load desired modules and data-files
			...
			if (url != '') {
				...
				await root.resourcesImport(url, {
					create: 'script',
		...
		smartSearch: {
			lookup: async (userInput, dataBaseObject, additionalCondition) => {
				...
				let filter = [],
					...
					quoted_queries = [],
					sanitizeRegEx = /[^a-zA-Z0-9äÄöÖüÜß\+\-\s]/g,
				...
				if (quoted_queries) {
					for (let i = 0; i < quoted_queries.length; i++) { // sanitize
				...
				processedUserInput.replace(quoted, '').toLowerCase().replace(sanitizeRegEx, '').split(/[\s]/g).forEach((term) => {
				...
				function fuzzy(haystack, needle, fuzzy, ratio) {
					...
					for (let sp = 0; sp <= haystack.length - needle.length; sp++) {
						// haystack
						// haysta
						//  aystac
						//   ystack
						...
						for (let i = 0; i < needle.length; i++) {
							...
							//     l
							//      e
							charPos = haystack.substring(sp, sp + needle.length).indexOf(needle[i]);
				...
				if (query.length > 0) {
					...
					let fuzzySearch = await core.fn.async.memory.read('coreFuzzySearch') || Boolean(userInput.match(fuzzyOverride)),
						coreFuzzyThreshold = await core.fn.async.memory.read('coreFuzzyThreshold');
					let fuzzyRatio = 2 - ((coreFuzzyThreshold || 5) * .1); //fuzzy ratio of 1.5 by default is quite reasonable determined through trial and error
					Object.keys(dataBaseObject).forEach((key) => {
						let haystack = dataBaseObject[key],
							filtered = false;
						...
						if (!filtered) {
							...
							query.forEach(function (needle) {
								let fuzzyquery = fuzzy(haystack, needle, fuzzySearch, fuzzyRatio);
				...
				found = found.sort(core.fn.static.sortBySecondColumn) || 0
		...
		stdout: async (where, what) => { //handles output, thus is suitable for unit-testing and debugging.
			...
			where.forEach(function (w) {
				if (coreOutputMonitor || w === 'console') {
					group = w + ' from ' + core.var.currentScope;
...
core.init = {
	...
	ui: async (query = '') => { //displays start screen
		...
		core.fn.async.stdout('temp',
			core.fn.static.lang('greeting') + '<br />' +
			(core.var.letterTemplate ? '<br /><a href="' + core.var.letterTemplate + '" target="_blank">' +
				core.fn.static.insert.icon('word') + core.fn.static.lang('openLetterTemplate') + '</a><br />' : '') +
...
core.setup = {
	...
	common: async () => {
		...
		if (typeof (core.var) !== 'undefined') {
			Object.keys(core.var.themes).forEach((key) => {
	...
	key: async () => { //  password construction - please forgive me the ugly nested javascript creation
		let display = '<form onsubmit="' +
			...
			core.fn.static.lang('settingKeyPassword0') + core.var.drm.pwLength.min + ' - ' + core.var.drm.pwLength.max + '<br /><input type="password" pattern=".{' + core.var.drm.pwLength.min + ',' + core.var.drm.pwLength.max + '}" required id="setupkeypassword0" /><br /><br />' +
			core.fn.static.lang('settingKeyPassword1') + '<br /><input type="password" required id="setupkeypassword1" /><br /><br />' +
...
core.globalSearch = { //searches all modules using their api-methods from the start page
	...
	display: (search = '') => {
		...
		if (Object.keys(core.globalSearch.result).length) {
			displayResult = '<br />';
			Object.keys(core.var.modules).forEach((mod) => {
				if (mod in core.globalSearch.result) {
					displayResult += '<div class="items items143" onclick="core.fn.static.toggleHeight(this)">' +
						core.fn.static.insert.expand() +
						core.var.modules[mod].icon + core.var.modules[mod].display[core.var.selectedLanguage] + '<br />';
					core.globalSearch.result[mod].sort(core.fn.static.sortBySecondColumn);
					core.globalSearch.result[mod].forEach(function (key) {
