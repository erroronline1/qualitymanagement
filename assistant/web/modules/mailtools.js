...
mailtools.api = {
	available: async (search) => {
		...
		found.forEach((value) => {
			display = '<a href="javascript:core.fn.async.loadScript(\'' + core.var.moduleDir + 'mailtools.js\',\'mailtools.fn.init(\\\'' + searchobject[value[0]][1] + '\\\')\')">' + searchobject[value[0]][0] + '</a>';
...
mailtools.fn = {
	...
	signatureinput: function () {
		core.fn.async.stdout('mailtoolgen', core.fn.static.insert.icon('refresh', 'bigger', false,
			'onclick="core.fn.async.stdout(\'output\', mailtools_data.signature[core.var.selectedLanguage]());" title="' + core.fn.static.lang('buttonGenTitle', 'mailtools') + '"'));
		core.fn.async.stdout('temp', mailtools_data.signature[core.var.selectedLanguage](1) +
			'<br /><br />' +
			core.fn.static.languageSelection('onchange="core.fn.async.stdout(\'output\', mailtools_data.signature[this.id]());"').join('<br />') +
	...
	notavailablegen: function () {
		...
		mailtools.var.disableOutputSelect = false;
		core.fn.async.stdout('output', mailtools_data.notavailableResponse['de'](dates) +
			mailtools_data.notavailableResponse['en'](dates));
	},
	init: async (query) => {
		if (typeof mailtools_data === 'undefined') await core.fn.async.loadScript(core.var.moduleDataDir + 'mailtools.js');
		let options = {
