...
ticketorder.api = {
	...
	processAfterImport: async (search) => {
		...
		if (typeof ticketorder_data !== 'undefined') {
			...
			data_without_header = JSON.parse(JSON.stringify(ticketorder_data));
			...
			//});
			if (found.length) {
				display = '<a href="javascript:core.fn.loadScript(\'' + core.var.moduleDir + 'ticketorder.js\',\'ticketorder.fn.init(\\\'' + search.replace(/"/g, '&quot;') + '\\\')\')">' + found.length + core.fn.static.lang('apiItemsFound', 'ticketorder') + '</a>';
	...
	search: function (query) {
		...
		if (typeof ticketorder_data !== 'undefined') {
			if (value(query) !== '') {
				...
				data_without_header = JSON.parse(JSON.stringify(ticketorder_data));
				data_without_header.content[0] = new Array(data_without_header.content[0].length);
				found = core.fn.smartSearch.lookup(query, data_without_header.content,
				...
				} else list = core.fn.staticlang('errorNothingFound', 'ticketorder', query);
