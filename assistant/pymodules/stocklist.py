...
''')
	...
	print ('<stocklist> as integrated module')
...
#          |_|
#
def open_import_filter(settings, module):
	global HEADERS
	global ANIMATION
	...
	if '/' in filename: # aka distinct file with path is passed
		directory = os.path.split(settings['source'])[0]
		filename = os.path.split(settings['source'])[1]
...
#
#
def translate(settings, imported, presets = None):
	global ANIMATION
	module = settings['module']['translate']

	ANIMATION = False
	if not presets and not choice('translate file to ' + module['destination'] + ' [y / n]?', ['y', 'n'])=='y':
		return 'cancel'
	...
	if len(result):
		...
		#################################################
		directory = os.getcwd()
		filename = module['destination'] 
		if '/' in settings['source']: # aka distinct file with path is passed
			directory = os.path.split(settings['source'])[0]
		if '/' in module['destination']:
			directory = os.path.split(module['destination'])[0]
			filename = os.path.split(module['destination'])[1]
		module['destination'] = os.path.join(directory, filename)
...
#      |_|
# 
def split(settings, imported, exportformat = None):
	global HEADERS
	global ANIMATION
	...
	if result:
		ANIMATION = False
		if not exportformat:
			exportformat = choice("Export to .csv or .xlsx [c / x]?", ["c", "x"])
		ANIMATION = True

		directory = os.getcwd()
		filename = module['prefix'] 
		if '/' in settings['source']: # aka distinct file with path is passed
			directory = os.path.split(settings['source'])[0]
		if '/' in module['prefix']:
			directory = os.path.split(module['prefix'])[0]
			filename = os.path.split(module['prefix'])[1]
		module['prefix'] = os.path.join(directory, filename)
...
#
# 
def start(settings, module, presets = None):
	global ANIMATION
	global RESULTSTRING
	RESULTSTRING = ''
	...
	imported = open_import_filter(settings, module)

	if not imported:
		return
	else:
		if module == 'translate':
			success = translate(settings, imported, presets)
		elif module == 'split':
			success = split(settings, imported, presets)
...
$ stocklist --help     for overview''')
	...
	if (not SETTINGS or confirm == 'h') and confirm != 'r':
		...
		start(SETTINGS, 'translate')
	elif confirm == 's':
		start(SETTINGS, 'split')
