	...
	function:{
		gen:function (){ //create user output
			if (typeof(JSONDATA)!='undefined'){
				...
				Object.keys(contents).forEach(function(value){ checkbox[value]=el('c'+value)?(el('c'+value).checked?1:0):1;});
				...
				//output
				el("output").innerHTML="";
				var index=0;	if (el('thirdperson').checked) index=1;
				var output='';
				Object.keys(contents).forEach(function(value){
						output+=contents[value][core.function.languageSynthesis.outputLanguage()][index].replace(/\$(\w+?)\$/ig,function(match,group1){return core.function.languageSynthesis.output(group1)});
